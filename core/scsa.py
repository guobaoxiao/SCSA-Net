...
class SCSA_module(nn.Module):
    def __init__(self, in_channel):
        ...
        self.attq = nn.Sequential(
            nn.Conv2d(in_channel, in_channel // 8, kernel_size=1)
        )
        ...
        self.convz = nn.Sequential(
            nn.InstanceNorm2d(2 * in_channel, eps=1e-3),
    ...
    def forward(self, x):
        ...
        # spatial attention
        q1 = self.attq(x1).squeeze(3)
        k1 = self.attk(x1).squeeze(3)
        v1 = self.attv(x1).squeeze(3)
        scores = torch.bmm(q1.transpose(1, 2), k1)
        att1 = torch.softmax(scores, dim=2)
        out1 = torch.bmm(v1, att1.transpose(1, 2))
        out1 = out1.unsqueeze(3)

        # channel attention
        q2 = x1.squeeze(3)
        scores2 = torch.bmm(q2, q2.transpose(1, 2))
        att2 = torch.softmax(scores2, dim=2)
        out2 = torch.bmm(att2, q2)
        out2 = out2.unsqueeze(3)

        # concatenation
        outz = self.convz(torch.cat([out1, out2], dim=1))
        out = x + self.gamma * outz
        return out
...
class SCSABlock(nn.Module):
    def __init__(self, net_channels, input_channel, depth, clusters):
        ...
        self.SCSA1 = SCSA_module(channels)   #SCSA Block 1
        ...
        self.SCSA2 = SCSA_module(channels)  # SCSA Block 2
        self.l2 = []
        for _ in range(self.layer_num // 2):
        ...
        # self.l1_2.append(attention(channels))
    ...
    def forward(self, data, xs):
        ...
        x1_att = self.SCSA1(x1_1)
        x1_1 = self.l1_1(x1_att)

        x_down = self.down1(x1_1)

        x2_att = self.SCSA2(x_down)
...
class SCSANet(nn.Module):
    def __init__(self, config):
        ...
        self.weights_init = SCSABlock(config.net_channels, 4 + self.side_channel, depth_each_stage, config.clusters)
        self.weights_iter = [SCSABlock(config.net_channels, 6 + self.side_channel, depth_each_stage, config.clusters) for
